	...
	def __init__(self, inPath, buffer=None):
		...
		with open(inPath, "r") as file:
			...
			if "," in header:
				...
				print(
					"Error: No valid delimiter (',' or ';') found in header of file {}...".format(inPath))
				sys.exit(1)

		self.init_args[self.get_args_index(self.init_args, "decoder")]["args"] = {
			...
			for drain_index, drain in enumerate(self.drains):
				...
				if output and outFiles[drain_index]:
					if writers[drain_index]:
						...
						writers[drain_index] = csv.DictWriter(
							outFiles[drain_index], fieldnames=fieldnames, lineterminator="\n")
